...
cd /tmp
unzip /tmp/hostingRanges.tsv.zip
rm -f /tmp/hostingRanges.tsv.zip
cd -
...
EXECUTE format('COPY (SELECT CASE WHEN iprange IS NOT NULL THEN iprange ELSE inet_merge(ipstart,ipend) END AS cidr FROM hosting WHERE provider = %L ORDER BY cidr) TO %L', rec.provider, filename);
END LOOP;
END $$;
EOF
